<?php 
	...
	function getSaleorders($receipt_id = '',$payment_type = '', $from_date ='', $to_date='', $shop='', $pageLimit='', $setLimit='', $export="", $order_type="")
	{
		$date = date('Y-m-d');
		$qry="SELECT * FROM sale_orders WHERE 1"; 	
	//	$order_type = (isset($order_type) && $order_type != '') ? $order_type : 'counter_sale';
	
		$qry .=" AND order_type = 'counter_sale'";
	...
	function displayPaginationBelows($per_page,$page,$receipt_id = '',$payment_type = '', $from_date ='', $to_date='', $shop='', $export="", $order_type="") {
	    $page_url="?";
		$query = "SELECT COUNT(*) as totalCount FROM sale_orders WHERE 1";
	
		$order_type = (isset($order_type) && $order_type != '') ? $order_type : 'counter_sale';
	
		$qry .=" AND order_type = '$order_type'";
	...
	$order_type = (isset($_GET['order_type']) && $_GET['order_type'] !='') ? $_GET['order_type'] : 'counter_sale';
								...
								<div class="box-body">
									<div id="form" class="panel panel-warning" style="display: block;">
										<div class="panel-body">
											<form action="counter_sale.php" accept-charset="utf-8">
												...
												<div class="row">
													...
													<div class="col-sm-3">
														<div class="form-group">
															...
															<select name="payment_type" id="payment_type" class="form-control select2" style="width: 100%;">
																...
																<option value="code" <?php if($payment_types == 'code') { echo "Selected"; } ?>>Cod</option> -->
															</select>
														</div>
													</div>
													<div class="col-sm-3">
													...
													<div class="col-sm-12">
														...
														<a href="counter_sale.php?order_type=<?php echo $order_type; ?>" class="btn btn-default">Reset</a>
													</div>
												</div>
											</form>
										</div>
									...
									<div class="row">
										...
										<tbody>
										    	<?php
											$tot_cost_price = $tot_price_val = $net_price = 0;
                                					$tot_profit_loss = 0;
                                					$grand_profit = 0;
                                					$grand_loss = $grand_total = 0;												
												...
												if($prs != false) {
													...
													if($pcount > 0) {
														...
														$receipt_id = $prow->receipt_id;
															...
															if($prs2 != false) {
																...
																if($pcount2 > 0) {
																	for($p2 = 0; $p2 < $pcount2; $p2++) {
																		...
																		$cost_price= $prow2->cost_price * $prow2->qty;
																		$price = $prow2->price * $prow2->qty;
																		$qty = $prow2->qty;
																		$price_val += $price;
										                                $cost_val += $cost_price;
																	}
                                                                    $tot_profit = $tot_loss = 0;
                                    								$tot_profit_loss = (($price_val - $prow->discount) - $cost_val);
                                    								if($tot_profit_loss > 0) {
                                    									$tot_profit = $tot_profit_loss;
                                    								} else {
                                    									$tot_loss = $tot_profit_loss;
                                    								}
                                    								if($prow->cash_back_receipt_id != '0' && $prow->cash_back_receipt_id != '') {
                                    									$cash_back_tot_amount = $prow->cash_back_tot_amount;
                                    								} else {
                                    									$cash_back_tot_amount = 0;
                                    								}
																}
															}
															echo "<tr>";
															...
															echo "<td>".$prow->cash_back_receipt_id."/".$prow->cash_back_tot_amount."</td>";
															echo "<td align='right'>".number_format($cost_val, 2)."</td>";
															echo "<td align='right'>".number_format($price_val, 2)."</td>";
                                                            echo "<td align='right'>".number_format($discount, 2)."</td>";
                                                            echo "<td align='right'>".number_format($tot_profit, 2)."</td>";
                                                            echo "<td align='right'>".number_format($tot_loss, 2)."</td>";
                                                            if(BILL_TAX == 'yes' && BILL_COUNTRY == 'UAE') {
															    echo "<td align='right'>".number_format(($price_val * 0.05), 2)."</td>";
															    echo "<td align='right'>".number_format((($price_val + ($price_val * 0.05)) - ($discount + $cash_back_tot_amount)), 2)."</td>";
                                                            } else {
                                                                echo "<td align='right'>".number_format(($price_val - ($discount + $cash_back_tot_amount))/21, 2)."</td>";
                                                                echo "<td align='right'>".number_format(($price_val - ($discount + $cash_back_tot_amount)), 2)."</td>";
                                                            }
															if($status == 'deleted'){
																echo "<td></td>";
															}else{
																echo "<td class='hide_print'>";
																	echo "<div class=\"text-center\">";
																		echo "<div class=\"btn-group\">";
																			echo "<a href=\"javascript:void(0)\" data-toggle=\"modal\" data-target=\"#myModal".$id."\" title=\"View Items\" class=\"tip btn btn-warning btn-xs\"><i class=\"fa fa-eye\"></i></a>";
															...
															echo "</tr>";
															$grand_total += $cash_back_tot_amount;
                                    						$grand_profit += $tot_profit;
                                    						$grand_loss += $tot_loss;
                                    						$tot_cost_price += $cost_val;
                                    						$tot_price_val += $price_val;
                                    						if(BILL_TAX == 'yes' && BILL_COUNTRY == 'UAE') {
                                    							$net_price += (($price_val + ($price_val * 0.05)) - ($discount + $cash_back_tot_amount));
                                    						} else {
                                    							$net_price += ($price_val - ($discount + $cash_back_tot_amount));
                                    						}
														...
														echo "<td align='right'>".number_format($tot_cost_price, 2)."</td>";
                                    					echo "<td align='right'>".number_format($tot_price_val, 2)."</td>";
                                    					echo "<td></td>";		
                                    					echo "<td align='right'>".number_format($grand_profit, 2)."</td>";
                                    					echo "<td align='right'>".number_format($grand_loss, 2)."</td>";										
                                    					echo "<td></td>";				
                                    					echo "<td align='right'>".number_format($net_price, 2)."</td>";
                                    					echo "<td></td>";
														echo "</tr>";
													} 
													else {
