...
$table .= '<table border="1" cellspacing="0" bordercolor="#222"><tr>'; 
		...
		$table .= '<td style="background-color:#244062; color:#fff;">CGST</td>';
		$table .= '<td style="background-color:#244062; color:#fff;">SGST</td>';
		} }
		$table .= '<td style="background-color:#244062; color:#fff;">Final Amt</td>';
        $table .= '</tr>';

		$grand_total = $total11 = $total12 = $total_vat =  $grand_total_gst = "0.00";												
...
if($prs != false) {
	...
	if($pcount > 0) {
		for($p = 0; $p < $pcount; $p++) {
			...
			$total = $total_sgst =$total_cgst = $cgst= $sgst= "0.00";
			$grand_total = '';
			...
			if($prs2 != false) {
				...
				if($pcount2 > 0) {
					...
					$total_sgst = ($total*$prow2->SGST)/100;
					$total_cgst = ($total*$prow2->CGST)/100;
					
				}
			}

			$total_cgst += $combo_package_price/100*$cgst;
			$total_sgst += $combo_package_price/100*$sgst;

			$grand_total = $with_vat_price-$discount;
			$total_with_gst = $total+ $total_cgst + $total_sgst - $discount; 
			...
			$grand_total_gst += $total_with_gst;
        ...
        $table .= "<tr>";
			...
			$table .=  "<td>".$vat_price."</td>";
				...
				$table .=  "<td>".$total_sgst."</td>";
				$table .=  "<td>".$total_cgst."</td>";
				} } 
			if(BILL_TAX == 'yes'){ if(BILL_TAX_TYPE == 'VAT'){
			$table .=  "<td style='background-color:#ffff00;'>".$grand_total."</td>";
			}elseif(BILL_TAX_TYPE == 'GST'){
			$table .=  "<td style='background-color:#ffff00;'>".$total_with_gst."</td>";
			} }	
			$table .=  "</tr>";
		}
		$table .=  "<tr>";
		$table .=  "<td colspan='10'>Final balance</td>";
		if(BILL_TAX == 'yes'){ if(BILL_TAX_TYPE == 'VAT'){ 
		...
		$table .=  "<td colspan='1' style='background-color:#ffff00;'>".number_format($total12, 2)."</td>";
			...
			$table .=  "<td colspan='1' style='background-color:#ffff00;'>".number_format($grand_total_gst, 2)."</td>";
		}}
		$table .=  "</tr>";
		} }
		$table .= '</table>';
