...
//echo $query;
   ...
   public function getDriverName($driver_id)
   {
	...
	public function setSettleSale($inputs){//echo '000'; die;
		...
		cg_recover,gross_total,discount,net_total,cash_drawer,settle_date,total_vat) 
	...
	public function getSaleOrderItemDetailsList($inputs) {//print_r($inputs); die;
		...
		if ($result) {
			...
			while ($row = mysqli_fetch_assoc($result)) {
				...
				$result_ar['combo_package_name']=$row['combo_package_name'];
	...
	// For Sale Order Edit
	function getLastSettleDate()
	{
		$where = "WHERE 1 ORDER BY ID DESC LIMIT 1 ";
		$settle_date = $this->getnamewhere('settle_sale', 'settle_date', $where);
		return $settle_date;
	}
