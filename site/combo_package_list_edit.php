...
function getComboID($name)
{
	$where = "WHERE package_name = '$name'";
	$service = getnamewhere('combo_package', 'id', $where);
	return $service;
}
...
if($main_id != '' || $sale_order_id != '' || $name != '') {
	$pack_price = getcomboprice($main_id);

	if($name != '')
	{
		$main_id = getComboID($name);
...
$result = mysqli_query($GLOBALS['conn'], "SELECT * FROM ".DB_PRIFIX."sale_orders WHERE id = '$sale_order_id' AND order_type = 'combo' ORDER BY id ASC");  
	$sale_order =  mysqli_fetch_assoc($result);
	$pack_price = $sale_order['combo_package_price'];
	// print_r($sale_order);
	...
	$old_item_ids = explode(',', $old_item_ids);
	// echo '<pre>';print_r($old_item_ids);echo '</pre>';
?>
<!DOCTYPE html>
<!--[if lt IE 7]>      
...
// echo '<pre>';print_r($items);die;
					...
					foreach($items as $item) {
					 ?>
						<option value="<?php echo $item['id']; ?>" 
							<?php if($old_item_ids[$i] == $item['id']) { echo 'selected'; } ?>>
							<?php echo $item['name']; ?> - <?php echo $item['price'].' '.CURRENCY; ?>						
						</option>
					<?php } ?>
				</select>
		  ...
		  <input style="margin-bottom:10px;" class="form-control" name="contact_number" id="contact_number" value="<?php echo $sale_order['contact_number']; ?>" type="number" placeholder="Customer Number" required/>
		  <div id="suggesstion-box"></div>
		  <input style="margin-bottom:10px;" class="form-control" name="contact_name" id="contact_name" value="<?php echo $sale_order['contact_name'];?>" type="text" placeholder="Customer Name" required/>
		  <input style="margin-bottom:10px;" class="form-control" name="address" id="address" value="<?php echo $sale_order['address'];?>" type="text" placeholder="Customer Address"/>
		  <hr>
