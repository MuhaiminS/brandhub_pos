...
{
	$date = date('Y-m-d');
	$qry="SELECT *, DATE_FORMAT(settle_date,'%Y-%m-%d') as settle_dat FROM settle_sale WHERE DATE_FORMAT(settle_date,'%Y-%m-%d') >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH)"; 
...
function displayPaginationBelows($per_page,$page,$from_date ='', $to_date='', $shop='', $export) {
    $page_url="?";
	$date = date('Y-m-d');
	$query = "SELECT COUNT(*) as totalCount FROM settle_sale WHERE DATE_FORMAT(settle_date,'%Y-%m-%d') >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH)";	
	if($shop != ''){
...
<div id="main" class="container-fluid">
				...
				<div class="row">
					<div id="breadcrumb" class="col-xs-12">
						...
						<ol class="breadcrumb pull-left">
							...
							<li><a href="javascript:void(0);">Monthly Settle Sale Reports
							</a></li>
						</ol>
					</div>
				</div>
				  ...
				  <div class="tab-content">
					<div id="counter_sale" class="tab-pane fade in active">
						<div class="row">			
							...
							<form action="monthly_settle_sale.php?export='1'">
							<div class="row search-st">
								...
								<div class="form-group search_val" style="margin-bottom:0px;">	
									...
									<div class="col-sm-3" title="Reset Search"><a href="monthly_settle_sale.php" class="print2"><i class="fa fa-refresh" ></i></a>
								...
								<div class="box">
									<div class="box-header">
										<div class="box-name">
											...
											<span>Monthly Settle Sales List</span>
										</div>
										<div class="box-icons">
	...
	<table class="table table-striped table-bordered table-hover table-heading no-border-bottom" border="1px solid Black">	
		<thead>
			<tr><th>Monthly Settle Sale Reports</th></tr>
...
jQuery(function($) {    
	...
	$(".excel_me").on('click', function (event) {
		 exportTableToCSV.apply(this, [$('#counter_sale_orders_export>table'), 'export_monthly_settle_sales.csv']);
	});
});
