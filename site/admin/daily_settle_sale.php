...
{
	$date = date('Y-m-d');
	$qry="SELECT *, DATE_FORMAT(settle_date,'%Y-%m-%d') as settle_dat FROM settle_sale WHERE id != ''"; // WHERE DATE_FORMAT(settle_date,'%Y-%m-%d') = '$date'"; 
...
function displayPaginationBelows($per_page,$page,$from_date ='', $to_date='', $shop='', $export) {
    $page_url="?";
	$date = date('Y-m-d');
	$query = "SELECT COUNT(*) as totalCount FROM settle_sale WHERE id != ''"; // WHERE DATE_FORMAT(settle_date,'%Y-%m-%d') = '$date'";	
	if($shop != ''){
				...
				<div class="row">
					<div id="breadcrumb" class="col-xs-12">
						...
						<ol class="breadcrumb pull-left">
							...
							<li><a href="javascript:void(0);">Daily Settle Sale Reports
							</a></li>
						</ol>
					</div>
				</div>
				  ...
				  <div class="tab-content">
					<div id="counter_sale" class="tab-pane fade in active">
						<div class="row">			
							...
							<form action="daily_settle_sale.php">
							<div class="row search-st">
								...
								<div class="form-group search_val" style="margin-bottom:0px;">	
									...
									<div class="col-sm-3" title="Reset Search"><a href="daily_settle_sale.php" class="print2"><i class="fa fa-refresh" ></i></a>
								...
								<div class="box">
									<div class="box-header">
										<div class="box-name">
											...
											<span>Daily Settle Sales List</span>
										</div>
										<div class="box-icons">
...
<div class="box-content no-padding" id="counter_sale_orders_export" style="display:none;">	
	<table class="table table-striped table-bordered table-hover table-heading no-border-bottom" border="1px solid Black">	
		<thead>
			<tr><th>Daily Settle Sale Reports</th></tr>
		...
		<tbody style="background: #fff none repeat scroll 0 0 !important;color: #525252;">
			<?php	$grand_total = "0.00";												
				$prs = getSettleSales($from_date1, $to_date1,$shops, $pageLimit='', 0, $export='1'); 
				if($prs != false) {
					$pcount = mysql_num_rows($prs);
					if($pcount > 0) {
						for($p = 0; $p < $pcount; $p++) {
...
jQuery(function($) {    
	...
	$(".excel_me").on('click', function (event) {
		 exportTableToCSV.apply(this, [$('#counter_sale_orders_export>table'), 'export_daily_settle_sales.csv']);
	});
});
