...
{
	$date = date('Y-m-d');
	$qry="SELECT * FROM sale_orders WHERE order_type = 'combo'"; 
	if($receipt_id != ''){
...
function displayPaginationBelows($per_page,$page,$receipt_id = '',$payment_type = '', $from_date ='', $to_date='', $shop='', $export="") {
    $page_url="?";
	$query = "SELECT COUNT(*) as totalCount FROM sale_orders WHERE order_type = 'combo'";
	if($receipt_id != ''){
				...
				<div class="row">
				  ...
				  <ul class="nav nav-tabs">
					<li><a href="manage_sale_orders.php">Counter Sale</a></li>
					 <li class="active"><a href="combo_sale.php">Combo Sale</a></li>
				  </ul>
					...
					<div id="counter_sale" class="tab-pane fade in active">
						<div class="row">			
							...
							<div class="form-group search_val" style="margin-bottom:0px;">	
								...
								<a href="combo_sale.php" style="font-size: 20px;" class="aa-search-btn reset_btn" title="Reset"><i class="fa fa-repeat" ></i></a>
								    ...
								    <span title="Excel" class="print2" style="font-size: 20px;"><a target="_blank" href="excel_export.php?sale=Counter Sale&order_type=combo&get_type=excel&receipt_id=<?php echo $receipt_ids; ?>&shop=<?php echo $shops; ?>&payment_type=<?php echo $payment_types; ?>&from_date=<?php echo $from_date1; ?>&to_date=<?php echo $to_date1; ?>" class="print excel_me"><i class="fa fa-file-excel-o"></i></a></span>
								    <?php } else { ?>
    								<span title="Excel" class="print2" style="font-size: 20px;"><a target="_blank" href="excel_export.php?sale=Counter Sale&order_type=combo&get_type=excel" class="print excel_me"><i class="fa fa-file-excel-o"></i></a></span>
    								<?php } ?>
								</div>
							</div>
							</form>
							<div class="col-xs-12">
															...
															for($p = 0; $p < $pcount; $p++) {
																...
																if($prs2 != false) {
																	...
																	if($pcount2 > 0) {
																		...
																		$vat_price = (($combo_package_price / 100) * ($vat));
																		$with_vat_price = $combo_package_price + (($combo_package_price / 100) * ($vat));
																	}
																}
																echo "<tr>";
																echo "<td>".$id."</td>";
																...
																echo "<td>".number_format($combo_package_price, 2)."</td>";
																echo "<td>".$vat_price."</td>";
																echo "<td>".number_format($with_vat_price-$discount, 2)."</td>";												
																if($status == 'deleted'){
																echo "<td></td>";/*"<td><a href='javascript:void(0)' onclick='deleteIt($id);'>Delete</a></td>";*/
																}else{
																echo " <td class='hide_print'><a data-toggle='modal' data-target='#myModal".$id."' style='cursor:pointer;'><i class='fa fa fa-eye'></i></a> | <a href='single_item_print.php?id=$id&re=combo_sale.php'><i class='fa fa-print'></i></a></td>";
																}
																echo "</tr>";
								...
								<?php echo displayPaginationBelows($setLimit, $page, $receipt_ids,$payment_types,$from_date1, $to_date1,$shops, $export=""); ?>
							</div>
						</div>
					</div>
				</div>
