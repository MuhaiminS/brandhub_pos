...
{
	$date = date('Y-m-d');
	$qry="SELECT * FROM sale_orders WHERE order_type = 'delivery'";
	if($shop != ''){
...
function displayPaginationBelows($per_page,$page,$receipt_id = '',$payment_type = '', $from_date ='', $to_date='', $shop='') {
    $page_url="?";
	$query = "SELECT COUNT(*) as totalCount FROM sale_orders WHERE order_type = 'delivery'";
	if($receipt_id != ''){
			...
			<div id="">
				...
				<div class="row">
				  ...
				  <ul class="nav nav-tabs">
					...
					<li class="active"><a href="delivery_sales.php">Delivery Sale</a></li>	
				  </ul>
						...
						<div class="row">			
							...
							<div class="form-group search_val" style="margin-bottom:0px;">	
								...
								<a href="manage_sale_orders.php" style="font-size: 20px;" class="aa-search-btn reset_btn" title="Reset"><i class="fa fa-repeat" ></i></a>
								    ...
								    <span title="Excel" class="print2" style="font-size: 20px;"><a target="_blank" href="excel_export.php?sale=Delivery Sale&order_type=delivery&get_type=excel&receipt_id=<?php echo $receipt_ids; ?>&shop=<?php echo $shop; ?>&payment_type=<?php echo $payment_types; ?>&from_date=<?php echo $from_date1; ?>&to_date=<?php echo $to_date1; ?>" class="print excel_me"><i class="fa fa-file-excel-o"></i></a></span>
								    <?php } else { ?>
    								<span title="Excel" class="print2" style="font-size: 20px;"><a target="_blank" href="excel_export.php?sale=Delivery Sale&order_type=delivery&get_type=excel" class="print excel_me"><i class="fa fa-file-excel-o"></i></a></span>
    								<?php } ?>
								</div>
							</div>
							</form>
													...
													if($prs != false) {
														...
														if($pcount > 0) {
															for($p = 0; $p < $pcount; $p++) {
																...
																echo " <td class='hide_print'><a data-toggle='modal' data-target='#myModal".$id."' style='cursor:pointer;'><i class='fa fa fa-eye'></i></a> | <a href='single_item_print.php?id=$id&re=delivery_sales.php&deliver=yes'><i class='fa fa-print'></i></a></td>";
																}
																echo "</tr>";
