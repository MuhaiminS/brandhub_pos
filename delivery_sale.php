   ...
   <body>
			...
			<div class="col-sm-4">
			   ...
			   <div class="value-split">
	                ...
	                <?php } elseif (BILL_TAX_TYPE == 'GST') { ?> 
	                    <span class="fn--amt">SGST:  </span><span class="tt-amount vat_total_sgst_tt">0.00</span><hr>
	                    <span class="fn--amt">CGST:  </span><span class="tt-amount vat_total_cgst_tt">0.00</span><hr>
						<?php }
						} ?>
				  ...
				  <div class="search-box">
							 ...
							 <div class="input-append span12">
								...
								<select id='standard' name='standard' class='custom-select'>
								  ...
								  <?php foreach($items as $item) {?>										
										<option data-name="<?php echo $item['name']; ?>" data-price="<?php echo $item['price']; ?>" data-cgst = "<?php echo $item['CGST']; ?>" data-sgst = "<?php echo $item['SGST']; ?>" value="<?php echo $item['id']; ?>"><?php echo $item['name']; ?> - <?php echo $item['barcode_id']; ?> - (<?php echo $item['price']; ?>)</option>
								  <?php } ?>
								</select>
							 </div>
						  </form>
					 ...
					 <div class="tab-content clearfix main-tb">						
						...
						if($item['category_slug'] == $cat['category_slug']) { ?>
							<div class="col-sm-2 <?php if($item['image']) { ?> prod-fas <?php } else { ?> prod-fas1 <?php } ?> prod-main" data-id = "<?php echo $item['id']; ?>" data-cgst = "<?php echo $item['CGST']; ?>" data-sgst = "<?php echo $item['SGST']; ?>" data-name = "<?php echo $item['name']; ?>" data-price = "<?php echo $item['price']; ?>">
							<?php if($item['image']) { ?>
							 ...
							 <span><img style="cursor:pointer;" data-id = "<?php echo $item['id']; ?>" data-name = "<?php echo $item['name']; ?>" data-price = "<?php echo $item['price']; ?>"  data-cgst = "<?php echo $item['CGST']; ?>" data-sgst = "<?php echo $item['SGST']; ?>" class="img-responsive" src="<?php echo $item_img_dir.$item['image']; ?>" alt="<?php echo $item['name']; ?>" title="<?php echo $item['name']; ?>"></span>
							  <?php } else { ?>
    ...
    <link href='css/jquery-customselect.css' rel='stylesheet' />
	  ...
	  $(".prod-main").click(function () {
			...
			add_product_to_list(id, name, unit_price,cgst,sgst);
		});

		function add_product_to_list(id, name, unit_price,cgst,sgst) {
			var qty = "1";
			...
			var list_val = "<tr><td style='width: 10%;'><input class='item_scroll"+id+"' type='hidden' name='items[]' value="+id+"><input class='cgst"+id+"' type='hidden' name='cgst[]' value="+cgst+"><input class='sgst"+id+"' type='hidden' name='sgst[]' value="+sgst+"></td><td style='width: 40%;'>"+name+"</td><td style='width: 30%;'>"+minus_symbol+"<input style='width: 50px;' class='quantity' name='quantity[]' type='number' value="+qty+">"+plus_symbol+"</td><td style='width: 10%; text-align: right;'>"+unit_price+"</td><td style='width: 10%; text-align: right;'>"+price+"</td></tr>";
	  ...
	  //Minus button
		...
		function show_sub_total_new(){
			...
			numRows = $(".listing--item tr").length;
			for(var i=1 ; i<numRows ; i++){
				var total_price_tt = parseFloat($("tr:nth-child(" + i + ") td:nth-child(5)").text());	
			<?php if (BILL_TAX == 'yes') {
			if (BILL_TAX_TYPE == 'GST') { ?>
			    //alert($(".listing--item tr:nth-child(" + i + ")").data('cgst'));
			    var cgst = parseFloat($(".listing--item tr:nth-child(" + i + ") td:nth-child(1) input:nth-child(2)").val()).toFixed(2);
			    var sgst = parseFloat($(".listing--item tr:nth-child(" + i + ") td:nth-child(1) input:nth-child(3)").val()).toFixed(2);
			    //alert(cgst +'----'+sgst);
			    total_cgst += parseFloat(total_price_tt) - (parseFloat(100*total_price_tt) / (100+parseFloat(cgst)));
			    total_sgst += parseFloat(total_price_tt) - (parseFloat(100*total_price_tt) / (100+parseFloat(sgst)));
			    //var total_sgst +=  total_price_tt * parseFloat(sgst/100);
			    // alert(total_cgst + '-------' + total_sgst );
			<?php }
			} ?>
				total_price_val += total_price_tt;
			}
			...
			var grand_total_sub = Math.round(total_price_val - discount_total);
				...
				<?php } else { ?>
					var vat_total_sub = ((net_total_sub / 21)).toFixed(2);
			...
			$('.net_total_tt').html((net_total_sub-total_cgst-total_sgst).toFixed(2));
			<?php if (BILL_TAX == 'yes') {
			if (BILL_TAX_TYPE == 'VAT') { ?>
			$('.vat_total_tt').html(vat_total_sub);
			<?php } else if (BILL_TAX_TYPE == 'GST') { ?>
			$('.vat_total_cgst_tt').html(total_cgst.toFixed(2));
			$('.vat_total_sgst_tt').html(total_sgst.toFixed(2));
			<?php }
			} ?>
