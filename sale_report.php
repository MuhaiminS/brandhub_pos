				  ...
				  <tbody>
							...
							<tr>
								...
								<?php } 
								 elseif(BILL_TAX_TYPE=="GST")  { 
									$total_amount = $sales_rep['combo_package_price'];
									$cgst = $sales_rep['combo_package_gst']/2;
									$sgst = $sales_rep['combo_package_gst']/2;
									$combo_total_cgst = $total_amount/100*$cgst;
			   						$combo_total_sgst = $total_amount/100*$sgst;
			   						$total_gst = $combo_total_cgst+$combo_total_sgst; ?>

									<td><?php echo number_format($total_gst,2); ?></td>
									<td><?php echo number_format(($total_gst+$total_amount),2); ?></td>
								<?php	}
									}
								else { ?>
					 ...
					 <tr>
						...
						<?php } 
						 elseif(BILL_TAX_TYPE=="GST")  { 
							$total_amount = $price;
							$total_cgst += $total_amount - ((100*$total_amount) / (100+$item->CGST));
	   						$total_sgst += $total_amount- ((100*$total_amount) / (100+$item->SGST));
	   						$total_gst = $total_cgst+$total_sgst; ?>

							<td><?php echo number_format($total_gst,2); ?></td>
						<td><?php echo number_format(($price - $total_gst),2); ?></td>
						<?php } 
						} else { ?>
