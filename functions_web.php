...
function getItemsList($limit = 0, $order_by = "ASC")
{
	...
	while($row = mysqli_fetch_array($run)) {
		...
		$items[$id]['CGST'] = $row['CGST'];
		$items[$id]['SGST'] = $row['SGST'];
...
$combo_package_gst = (isset($inputs['gst']) && $inputs['gst'] !='') ? $inputs['gst'] : '';


 // print_r($combo_package_gst); die;
...
ordered_date,payment_status,status,amount_given,balance_amount,remarks, card_num, vat, combo_package_price, combo_package_name,combo_package_gst) 
VALUES('$table_id', '$floor_id', '$num_members', '$shop_id', '$user_id', '$customer_id', '$order_type', '$payment_type', '$discount' , '$contact_name', '$contact_number', '$address','$driver_id','$ordered_date','$payment_status','$status','$amount_given','$balance_amount','$remarks', '$card_num', '$vat', '$combo_package_price', '$combo_package_name', '$combo_package_gst')");
if ($result) {            
